<?php
    ...
    if ($conn) {
        if (isset($userID)) {
            ...
            if($action == 'add'){
                //Get the current quantity of product in the cart
                $sql = "SELECT Quantity from anonymous_$anonymousID WHERE ProductID=$id";
                $result = mysqli_query($conn, $sql);
                $row = mysqli_fetch_row($result);
                $quantity = $row[0];
                //if the quantity in cart is same with the stock (aka. cannot add anymore)
                if($quantity == $stock){
                    ...
                    $sql = "UPDATE anonymous_$anonymousID SET Quantity=Quantity+1 WHERE ProductID=$id";
                    $result = mysqli_query($conn, $sql);
                    $sql = "SELECT * FROM anonymous_$anonymousID WHERE ProductID=$id";
                    $result = mysqli_query($conn, $sql);
                    $row = mysqli_fetch_row($result);
                    echo "$row[3]";
                }
                ...
                $row = mysqli_fetch_row($result);
                if($row[3] == 0){
                    $sql = "DELETE FROM anonymous_$anonymousID WHERE ProductID=$id";
                    $result = mysqli_query($conn, $sql);
                }
                echo "$row[3]";
            }else if($action == 'remove'){
